<template id="planet-detail">
    <div v-if="planet" class="detail-planet-container">
        ...
        <p>
            <a class="button" :href=`/api/planet-system-by-name/${planet.centralCelestialBody.name}/planets/${planet.name}/delete`>Delete</a>
            <a class="button" :href=`/planet-systems/${this.planetSystemName}/planets/${planet.name}/update`>Edit</a>
        </p>
    </div>
</template>
<script>
    Vue.component("planet-detail", {
        ...
        created() {
            ...
            console.log("Planet system id: " + this.planetSystemName);
            const planetId = this.$javalin.pathParams["planet-id"];
            fetch(`http://localhost:7000/api/planet-systems/${planetSystemId}/planets/${planetId}`)
